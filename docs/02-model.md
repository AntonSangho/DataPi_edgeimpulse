...
> 상태: **테스트 95.2%** (2026-08-06) — 3-class / 창 3000 ms / `Normalize features` on
> 1차(4-class, 창 2000, norm off)는 64.9%였다. 중간 단계를 모두 남긴다.
> 다음: Phase 3 착수, 학습 세션 다양화 (세션은 아직 1개다)
...
> **2026-08-06: 켰다.** 테스트 88.3% → 91.8%, `wave` F1 0.92 → 1.00.
> 위 표의 창 2000 ms도 **3000 ms로 바꿨다.** 아래 A/B 절 참고.
> 이 절의 표는 1~2차 학습 시점의 설정으로 남겨 둔다.
...
> **이 가설은 반증됐다** (2026-08-06, 아래 A/B 절). 창을 안 바꾼 2-A에서
> 이 8창이 0이 됐다. 원인은 창 길이가 아니라 `Normalize features`였다.
...
> 만들어지는 규칙도 명시하지 않는다.**
>
> **Studio에서 직접 읽은 값은 `0.6`이다** (2026-08-06 확인). 문서에 없으므로
> 화면에서 확인하는 수밖에 없다 — 절차서에 넣을 항목이다.
...
## A/B 두 건 — `Normalize features`와 창 3000 ms (2026-08-06)

둘 다 **추가 수집 없이 몇 분**이면 재진다. 2차 학습에서 "조치 순서를 난이도가
아니라 비용 순으로 정렬했어야 했다"고 적었으므로 이것부터 했다.

**한 번에 하나씩** 바꿨다.

| | 창 | Normalize | 기준 대비 바뀐 것 |
|---|---|---|---|
| 기준 | 2000 ms | off | — |
| **2-A** | 2000 ms | **on** | Normalize만 |
| **2-B** | **3000 ms** | on | 2-A 대비 창만 |

2-A가 개선으로 판정됐으므로 그것을 새 기준선으로 놓고 창을 바꿨다.
2-B를 Normalize off로 했다면 두 변수가 동시에 움직여 `wave` 개선의 원인을
귀속할 수 없었을 것이고, **그 귀속이 아래 가설 반증의 핵심이다.**

> **안 잰 칸**: w3000 + Normalize **off**. 2×2를 채우려면 필요하지만 채택 후보가
> 아니라 건너뛰었다.

### 결과 (test 세트 90 lux, argmax 기준)

| | 기준 | 2-A | 2-B |
|---|---|---|---|
| 정확도 | 88.30% | 91.81% | **95.15%** |
| loss | 0.292 | 0.195 | **0.100** |
| AUC | 0.981 | 0.991 | **0.997** |
| 창 수 | 171 (57×3) | 171 | 165 (55×3) |
| `wave` F1 | 0.92 | **1.00** | **1.00** |
| `cover` recall | 49/57 | 48/57 | 49/55 |
| `idle` recall | 53/57 | 52/57 | 53/55 |
| `cover`↔`idle` 오분류 합 | 12창 | 14창 | 8창 |

원본: [`data/2026-08-06-metrics-3class-normalize.json`](data/2026-08-06-metrics-3class-normalize.json),
[`data/2026-08-06-metrics-3class-normalize-w3000.json`](data/2026-08-06-metrics-3class-normalize-w3000.json)

### ⚠️ `wave`의 "빈 창" 가설은 반증됐다 — 정정

2차 학습에서 `wave` → `idle` 8창을 이렇게 적었다:

> 2초 창 안에 손동작이 안 들어간 "빈 창"으로 보인다. (…) 창 3000 ms 시험의
> 근거가 여기 하나 더 생겼다.

**틀렸다. 2-A는 창이 2000 ms 그대로인데 `wave` 오분류가 8 → 0이 됐다.**
빈 창이 원인이었다면 창 길이를 안 바꾼 2-A에서 그대로 남아 있어야 했다.
고친 것은 창 길이가 아니라 `Normalize features`다.

실제 원인은 진폭 쪽으로 보인다 — train `wave`는 골 깊이 52%, test는 78%.
창별 정규화가 그 배율 차이를 지우자 붙었다. 1차 학습의 진단("세션이 하나뿐이라
그 60초의 진폭 패턴을 외웠다")과 방향이 같다.
**다만 정규화가 실제로 무엇을 하는지는 아직 문서로 확인하지 않았다 — 추론이다.**

이 가설이 반증된 것은 순전히 **변수를 하나씩 바꿨기 때문**이다.
2-B를 먼저 했거나 두 개를 같이 바꿨다면 "창을 늘리니 `wave`가 붙었다"는
틀린 결론을 문서에 박았을 것이다. 1차의 `Take log of spectrum` 오류와 같은 형태다.

### `cover`↔`idle` 숫자는 해석하지 않는다 — 창이 서로 독립이 아니다

12 → 14 → 8로 방향이 왔다갔다 한다. 여기에 인과를 붙이면 안 된다.

**stride가 500 ms이므로 연속한 창들이 데이터를 공유한다.**
2000 ms 창이면 4개, 3000 ms 창이면 6개가 겹친다.
즉 **잘못 분류된 2초 구간 하나가 창 3~4개를 만든다.** 12와 8의 차이는
독립 오류 12건과 8건이 아니라, 그런 구간 하나 차이일 수 있다.

> 이것은 1차의 "11.1% = 9창 중 1창"과 같은 함정의 다른 형태다.
> 그때는 **분모**가 작았고, 여기서는 분자가 **서로 상관되어 있다.**
> 창 개수는 독립 시행 횟수가 아니다.

대신 **loss(0.292 → 0.195 → 0.100)와 AUC(0.981 → 0.997)**는 연속값이라
창 상관에 덜 휘둘린다. 개선을 뒷받침하는 것은 이쪽이다.

### 채택

**창 3000 ms + `Normalize features` on.** 비용은 지연 2초 → 3초뿐이고,
리소스는 1차 기준 3 ms / 1.4 KB라 여유가 넘친다 (창이 바뀌었으므로 재측정 필요).

**다만 95.15%도 학습 세션 1개 위의 숫자다.** 정규화가 진폭 의존을 없앤 것이지
세션 다양성이 생긴 것은 아니다. 아래 수집 계획은 그대로 유효하다.
